...
class BouncingBallExample(nn.Module):
    ...
    def __init__(self, radius=0.2, gravity=9.8, adjoint=False):
        ...
        self.init_vel = nn.Parameter(torch.tensor([-5.0]))
        self.absorption = nn.Parameter(torch.tensor([0.1]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("nbounces", type=int, nargs="?", default=7)
    ...
    plt.ylim([velocity.min() - 0.02, max(trajectory.max(), velocity.max()) + 0.02])
